 ...
 */
public class BjyBackPlayDownManager {
    private static BjyBackPlayDownManager bjyPlayDownManager;
    public static BjyBackPlayDownManager Instance(){
        synchronized (BjyPlayDownManager.class){
            if(bjyPlayDownManager==null){
                bjyPlayDownManager=new BjyBackPlayDownManager();
            }
        }
    ...
    public void startDown(PlayDownConfig playDownBean) throws Exception {
        ...
        getDownloadManager().newPlaybackDownloadTask(playDownBean.getFileName(),playDownBean.getVideoId(),
                Long.parseLong(playDownBean.getSessionId()), playDownBean.getToken(),playDownBean.getuId()+","+playDownBean.getOccName()+","+playDownBean.getCourerName()+","+playDownBean.getSectionName())
        ...
        /*getDownloadManager().newPlaybackDownloadTask(playDownBean.getFileName(), playDownBean.getVideoId(),
                Long.parseLong(playDownBean.getSessionId()),
                playDownBean.getToken(), definitionList, playDownBean.getEncryptType(),
...
//        PlaybackDownloader playbackDownloader=new  PlaybackDownloader(BjyPlayDownConfig.newInstacne().getmActivity(), BjyPlayDownConfig.newInstacne().getBjyId(), BjyPlayDownConfig.newInstacne().getFilePath());
        //        //初始化下载
...
//        //读取磁盘缓存的下载任务并初始化加密参数
//        manager.loadDownloadInfo(BjyPlayDownConfig.newInstacne().getBjyId(), 1);
//        return manager;
//        return playbackDownloader;
                ...
                for(DownloadTask downloadTask:allVideo){
                    if(isFinsh){
                        if(downloadTask.getTaskStatus()==Finish){
                            ...
                            if(exx[0].equals(uid)&&exx[1].equals(occName)&&exx[2].equals(seachName)&&exx[3].equals(courseName)){
                                newAllVideo.add(downloadTask);
                            }
                        }
                    }else {
    ...
    public String getPlayBackSig(long rooid, long sessId){
        return  getDownloadManager().getTaskByRoom(rooid,sessId).getSignalFileName();
//        return getDownloadManager().getSignalPathByRoom((rooid),(sessId));
    }
}
