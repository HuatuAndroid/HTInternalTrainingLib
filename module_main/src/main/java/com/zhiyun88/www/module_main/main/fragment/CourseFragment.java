...
public class CourseFragment extends LazyFragment {
    ...
    public void initView() {
        topBarView.getCenterTextView().setText(R.string.main_my_course);
        topBarView.getLeftImageButton().setVisibility(View.GONE);

        ArrayList<String> str = new ArrayList<>();
        ArrayList<Fragment> mFragments = new ArrayList<>();
        str.add(getString(R.string.main_post_course));
        str.add(getString(R.string.main_optional_courses));

        mFragments.add(MyCourseFragment.newInstance(1));//岗位课程
        mFragments.add(MyCourseFragment.newInstance(2));//自选课程
